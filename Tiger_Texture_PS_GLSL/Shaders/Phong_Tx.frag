...
uniform bool u_is_godzilla = true;
uniform bool u_flag_texture_mapping = true;


uniform bool screen_flag = false;
uniform float screen_f = 1.0f; //for screen
uniform float screen_r = 0.1f;

uniform bool u_is_wolf = true;
uniform bool u_shadow_effect = true;
uniform int u_color_timer = 0;

in vec2 v_position_sc; //for screen
		...
		if (local_scale_factor > zero_f) {				
			...
			if(u_is_wolf){
				if(u_shadow_effect){
					if(u_color_timer == 0)
						color_sum.x = zero_f;
					else if(u_color_timer == 1)
						color_sum.y = zero_f;
					else if(u_color_timer == 2)
						color_sum.z = zero_f;
					else if(u_color_timer == 3){
						color_sum.x = zero_f;
						color_sum.y = zero_f;
					}
					else if(u_color_timer == 4){
						color_sum.y = zero_f;
						color_sum.z = zero_f;
					}
					else if(u_color_timer == 5){
						color_sum.x = zero_f;
						color_sum.z = zero_f;
					}
					else if(u_color_timer == 6){
						color_sum.x = zero_f;
						color_sum.y = zero_f;
						color_sum.z = zero_f;
					}
...
// For a normal rendering ...
	...
	if (screen_flag){
		float x, y;
		x = mod(v_position_sc.x*screen_f, 1.0f);
		y = mod(v_position_sc.y*screen_f, 1.0f);
		float r;
	...
	else if (u_flag_texture_mapping) 
